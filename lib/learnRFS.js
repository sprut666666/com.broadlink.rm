...
const start = (device, log, { progress, disableTimeout = false } = {}) => {
  ...
  if (!device.enterLearning) {
    return log(`Learn RFS (learning not supported for device at ${device.host.address})`)
  }
  if (!device.enterRFSweep) {
    return log(`Learn RFS (RFS learning not supported for device at ${device.host.address})`)
  }
  ...
  onRawData2 = (message) => {
    ...
    log(`Learn RFS (found frequency - 2 of 2)`)
    log(`[Press the RFS button multiple times with a pause between them]`);
